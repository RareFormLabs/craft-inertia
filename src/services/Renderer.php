...
class Renderer extends Component
{
    ...
    public function renderComponent(string $component, array $props = [], int $statusCode = 200, bool $handleErrors = true): Response
    {
        try {
    ...
    {
        [$page, $stringResponse] = $this->captureTemplateOutput($template, $this->injectCurrentElement($templateVariables));
    ...
    private function captureTemplateOutput(string $template, array $templateVariables = []): array
    {
        ...
        try {
            $stringResponse = Craft::$app->getView()->renderString(
                $this->processTemplatePulls($template),
                $templateVariables
            );
        } finally {
